...
import static org.onosproject.net.flow.instructions.Instruction.Type.L3MODIFICATION;
...
import static org.onosproject.pipelines.upflb.UpflbConstants.HDR_HDR_IPV4_DST_ADDR;
import static org.onosproject.pipelines.upflb.UpflbConstants.INGRESS_UPF_DNAT_TABLE;
import static org.onosproject.pipelines.upflb.UpflbConstants.INGRESS_UPF_SNAT_TABLE;
import static org.onosproject.pipelines.upflb.UpflbConstants.INGRESS_IP_ROUTE_TABLE;
import static org.onosproject.pipelines.upflb.UpflbConstants.INGRESS_PORT;
import static org.onosproject.pipelines.upflb.UpflbConstants.EGRESS_PORT;

/**
 * Interpreter implementation for upflb.p4.
 */
public class UpflbInterpreterImpl extends AbstractHandlerBehaviour implements PiPipelineInterpreter {
    ...
    private static final Map<Integer, PiTableId> TABLE_MAP = new ImmutableMap.Builder<Integer, PiTableId>()
            ...
            .put(0, UpflbConstants.INGRESS_UPF_DNAT_TABLE)
            .put(1, UpflbConstants.INGRESS_UPF_SNAT_TABLE)
            .put(3, UpflbConstants.INGRESS_IP_ROUTE_TABLE)
            .build();
    private static final Map<Criterion.Type, PiMatchFieldId> CRITERION_MAP = new ImmutableMap.Builder<Criterion.Type, PiMatchFieldId>()
    ...
    public Collection<PiPacketOperation> mapOutboundPacket(OutboundPacket packet) throws PiInterpreterException {
        ...
        // upflb.p4 supports OUTPUT and L3MODIFICATION instructions.
        List<OutputInstruction> outInstructions = treatment
                .allInstructions()
                .stream()
                .filter(i -> i.type().equals(OUTPUT) || i.type().equals(L3MODIFICATION))
                .map(i -> (OutputInstruction) i)
                .collect(toList());

        if (treatment.allInstructions().size() != outInstructions.size()) {
            // There are other instructions that are not of type OUTPUT or L3MODIFICATION.
    ...
    public InboundPacket mapInboundPacket(PiPacketOperation packetIn, DeviceId deviceId) throws PiInterpreterException {
        // Assuming that the packet is ethernet, which is fine since upflb.p4
        ...
        Optional<PiPacketMetadata> packetMetadata = packetIn.metadatas()
                .stream()
                .filter(m -> m.id().equals(INGRESS_PORT))
                .findFirst();
