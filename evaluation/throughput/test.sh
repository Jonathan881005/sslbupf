...
establish_sessions () {
	# For the convenience of conducting tests with diferent session number multiple times,
	# just establish all sessions in advance.
	MAX_UPF_NUMS=4
	if ! pgrep nr-ue; then
...
send_packets () {
	...
	for i in $(seq 1 ${UPF_NUMS}); do
		...
		set -x
		iperf3 -c 10.0.1.103 -p 520${i} -B ${UE_IP} ${IPERF3_ARGS} -t ${TIMESPAN} --timestamp > ${LOGS_PATH}/ue${i}-client.log 2>&1 &
