...
#${LIBGTP5GNL_PATH}/open5gs-gnb.sh
...
for round in $(seq 1 ${ROUND_COUNT}); do
	for ((i=${SESSION_MULTIPLIER}*(${round}-1)+1; i<=${SESSION_MULTIPLIER}*${round}; i++)); do
		echo "Start open5gs-ue${i}"
		set -x
		sudo -b ${UERANSIM_PATH}/build/nr-ue -c ${UERANSIM_PATH}/config/open5gs-ues/open5gs-ue${i}.yaml --no-routing-config
	...
	for ((i=${SESSION_MULTIPLIER}*(${round}-1)+1; i<=${SESSION_MULTIPLIER}*${round}; i++)); do
		set -x
		iperf3 -c 10.0.1.103 -p 520${i} -B 60.60.0.$((${i}+1)) -u -b 1G -l 1300 -t 250 --timestamp > ${LOGS_PATH}/ue${i}-client.log 2>&1 &
		set +x
		sleep 5
	done
